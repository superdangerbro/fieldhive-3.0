...
import { useJobStatuses, JobStatus } from './store';

interface EditDialogProps {
    open: boolean;
    status: JobStatus | null;
    onClose: () => void;
    onSave: (oldStatus: JobStatus | null, newStatus: JobStatus) => void;
}
...
export function JobStatusSection() {
    const { statuses, isLoading, error, fetch, update } = useJobStatuses();
    ...
    const handleDeleteStatus = async (statusToDelete: JobStatus) => {
        const updatedStatuses = statuses.filter(status => status.value !== statusToDelete.value);
        await update(updatedStatuses);
    };

    const handleSaveStatus = async (oldStatus: JobStatus | null, newStatus: JobStatus) => {
