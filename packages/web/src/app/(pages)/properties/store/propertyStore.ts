...
import { Property, CreatePropertyDto, UpdatePropertyDto, Address, PropertyResponse } from '@/app/globaltypes';
...
interface PropertyStore {
    ...
    // Account Actions
    getPropertyAccounts: (propertyId: string) => Promise<void>;
}
...
const convertToProperty = (data: any): Property => ({
    ...
    address: data.address ? {
        ...data.address,
        created_at: data.address.created_at ? new Date(data.address.created_at) : undefined,
        updated_at: data.address.updated_at ? new Date(data.address.updated_at) : undefined,
    } : undefined
});
...
export const usePropertyStore = create<PropertyStore>((set, get) => ({
    ...
    getPropertiesInBounds: async (bounds) => {
        try {
            ...
            const response = await fetch(
                `${BASE_URL}${PROPERTIES_ENDPOINT}/spatial/bounds?bounds=${west},${south},${east},${north}`
            );
    ...
    getPropertyAccounts: async (propertyId) => {
        try {
            ...
            // Could store accounts in state if needed
            return accounts;
        } catch (error) {
